...
class GANSolver(object):
    def __init__(self, config, len_dataset):
        ...
        if self.config.log_run:
            ...
            wandb.init(project="emotion-pix2pix",
    ...
    def sample_images(self, data_loaders):
        ...
        with torch.no_grad():
            ...
            if type(fake_B) is list:
                fake_B = fake_B[-1]
        grid_image_train = self._make_grid_image(
            self.real_A, self.real_B, fake_B).cpu()
        ...
        with torch.no_grad():
            ...
            if type(fake_B) is list:
                fake_B = fake_B[-1]
        grid_image_val = self._make_grid_image(
            self.real_A, self.real_B, fake_B).cpu()
    ...
    def set_inputs(self, inputs):
        ...
        if self.emoDB is not None and 'idx' in inputs.keys():
            ...
            self.emo_vec = torch.zeros(
                (self.real_A.size(0), 1)).to(self.device)
    ...
    def train_model(self, data_loaders, plot_grads=False):
        ...
        for i_epoch in range(1, self.config.num_epochs + 1):
            ...
            if self.config.log_run and i_epoch % self.config.save_interval == 0:
                ...
                save_image(img_sample, self.img_dir +
                           '/sample_{}.png'.format(i_epoch))
    ...
    def eval_model(self, data_loaders):
        ...
        real_img = torch.nn.functional.pad(
            real_img, [0, 30, 0, 0], mode='constant')
