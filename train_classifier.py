    ...
    def train_model(self,
        ...
        for i_epoch in range(1, self.config.num_epochs + 1):
            ...
            for phase in ['train', 'val']:
                ...
                self.epoch_acc = self.epoch_acc.double() / \
                    len(data_loaders[phase])

                # Logging
                if i_epoch % config.log_interval == 0:
...
if __name__ == '__main__':
    ...
    config = importlib.import_module(
        'configs.ravdess_im_classi').config
    ...
    # Train
    model = solver.train_model(data_loaders, writer)
